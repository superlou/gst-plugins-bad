...
main (int argc, char *argv[])
{
  ...
  intertest1->timer_id =
      g_timeout_add (2000, onesecond_timer_swap_channel, intertest1);

  intertest2 = gst_inter_test_new ();
  gst_inter_test_create_pipeline_receive (intertest2,
  ...
  gst_inter_test_create_pipeline_receive (intertest3,
      g_string_new ("channel2"));
  gst_inter_test_start (intertest3);
  intertest3->timer_id = g_timeout_add (1000, onesecond_timer, intertest3);
...
onesecond_timer_swap_channel (gpointer priv)
{
  ...
  intervideosink = intertest->sink_video_element;
  interaudiosink = intertest->sink_audio_element;

  g_object_get (G_OBJECT (intervideosink), "channel", video_channel, NULL);
  g_object_get (G_OBJECT (interaudiosink), "channel", audio_channel, NULL);

  if (strcmp (video_channel->str, "channel1") == STR_MATCH) {
    g_print ("Swapping to channel2.\n");
    g_object_set (G_OBJECT (intervideosink), "channel", "channel2", NULL);
    g_object_set (G_OBJECT (interaudiosink), "channel", "channel2", NULL);
  } else {
    g_print ("Swapping to channel1.\n");
    g_object_set (G_OBJECT (intervideosink), "channel", "channel1", NULL);
    g_object_set (G_OBJECT (interaudiosink), "channel", "channel1", NULL);
  }
